...
async def songs_info(seokey: str = Query(description="The `seokey` of the song. Example: `tyler-herro` in `https://gaana.com/song/tyler-herro`")):
    track_list = [seokey]
...
async def albums_info(seokey: str = Query(description="The `seokey` of the album. Example: `tyler-herro` in `https://gaana.com/album/tyler-herro`")):
    album_list = [seokey]
    album_info = await gaanapy.get_album_info(album_list, True)
    return album_info

@app.get("/artists/search/", summary="Search for artists.")
async def artists_search(query: str = Query(description="Name of the artist to search for."), 
                         limit: Optional[int] = 10):
    result = await gaanapy.search_artists(query, limit)
    return result

@app.get("/artists/info/", summary="Retrieve detailed information on an artist.")
async def artists_info(seokey: str = Query(description="The `seokey` of the artist. Example, `ksi` in `https://gaana.com/artist/ksi`")):
    artist_list = [seokey]
...
async def artists_similar(artist_id: str = Query(description="The `artist_id` of the artist. Example: `3519085`"), 
                          limit: Optional[int] = 10):
...
async def get_trending(language: str = Query(description="Available options: English, Hindi, Tamil, Punjabi etc."), 
                       limit: Optional[int] = 10):
...
async def get_new_releases(language: str = Query(description="Available options: English, Hindi, Tamil, Punjabi etc."), 
                           limit: Optional[int] = 10):
...
async def playlists_info(seokey: str = Query(description="The `seokey` of the playlist. Example: `gaana-dj-hindi-top-50-1` in `https://gaana.com/playlist/gaana-dj-hindi-top-50-1`")):
